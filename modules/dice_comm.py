    ...
    if arg == "help":
        ...
        if res_len == -1:
            ...
            if len(dice_string) > 2048:
                ...
                try:
                    await message.channel.send(f"**Your {dice_mode} result:**")
                    await message.channel.send(file=fileToSend)
                except discord.errors.Forbidden:
                    await forbiddenErrorHandler(message)
                finally:
                    try:
